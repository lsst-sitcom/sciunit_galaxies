...
if field == "cosmos":
    ...
    tract_patches = {
        tract: None
        for tract in (
            2078, 2079, 2080, 2081, 2233, 2234, 2235, 2236, 2237, 2392, 2393, 2394, 2395, 2396, 2397,
            2558, 2559, 2560, 2561, 2562, 2728, 2729, 2730, 2731,
        )
    }
    metrics_plot_band = {
        "vis": {
            "compl_24_25": {"vmin": 0.95, "vmax": 0.80},
            "mag_compl50": {"vmin": 25, "vmax": 26.4},
    ...
    tract_patches = {
        tract: None
        for tract in (2702, 2703, 2704, 2705, 2875, 2876, 2877, 2878, 3053, 3054, 3055)
    }
    # tract_patches = {tract: None for tract in (2702, 2703)}
...
dataset_tools = {
    ...
    dataset_type: {
        ...
        "ra": (MatchedRefCoaddDiffCoordRaTool, {"produce": kwargs_produce, "coord_ref": refcat_ra, "coord_ref_cos": refcat_dec}),
        "dec": (MatchedRefCoaddDiffCoordDecTool, {"produce": kwargs_produce, "coord_ref": refcat_dec}),
        "mag_cm odel": (MatchedRefCoaddDiffMagTool, {"produce": kwargs_produce}),
        ...
        "sersic_ra": (MatchedRefCoaddDiffCoordRaTool, {"produce": kwargs_produce, "coord_ref": refcat_ra, "coord_ref_cos": refcat_dec}),
        "sersic_dec": (MatchedRefCoaddDiffCoordDecTool, {"produce": kwargs_produce, "coord_ref": refcat_dec}),
    },
}
...
for dataset, tools in dataset_tools.items():
    ...
    for tract in tracts:
        ...
        if dataset.startswith("matched_"):
        ...
        for name, atool in tools_named.items():
            for band in bands:
                ...
                if (name == "completeness") and (band in bands_completeness):
                    print(f"doing completeness for {band=}")
                    ...
                    for key_metric, config_metric in metrics_plot.items():
                        ...
                        action.key = config_metric.key.format(
                            band=band,
                            name_flux_short=ref_matched.name_flux_short.format(band=band),
