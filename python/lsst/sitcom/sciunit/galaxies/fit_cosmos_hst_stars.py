...
class CatalogExposureCosmosHstStars(CatalogExposureCosmosHstBase):
    ...
    @classmethod
    def populate_psf_table(
        cls, table_psf_fits: Table,
        config_data: CatalogSourceFitterConfigData,
        catalog_ref: SourceCatalog, band: str | None = None,
    ):
        ...
        key_cen = config.get_prefixed_label(config.get_key_cen(), config.prefix_column)
        with Matcher(
            ref[f"{key_cen}{config.get_suffix_x()}"],
            ref[f"{key_cen}{config.get_suffix_y()}"],
        ) as matcher:
        ...
        for column in config_data.parameters.keys():
            column_full = f"{config.prefix_column}{column}"
