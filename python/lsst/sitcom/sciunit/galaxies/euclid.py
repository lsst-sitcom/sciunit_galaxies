...
datasettype_default = "euclid_coadd"
...
def get_cutouts_euclid(
    tract, patch, bands, skymap, position: SkyCoord, cutout_size, fits_euclid=None, keep_fits=False,
):
    ...
    for band in bands:
        if (fits_band := fits_euclid.get(band)) is None:
            ...
            fits_band = fits.open(f"{path_base}/{filename}.fits")
            if keep_fits:
                fits_euclid[band] = fits_band
        n_finite = 0
        cutout_best = None
        for hdu_cdfs in fits_band[1:]:
    ...
    for tract in tracts:
        ...
        for patchInfo in skymap[tract]:
            ...
            if len(matches) > 0:
                ...
                for mosaic_filename in mosaics:
                    hdu = fits.open(f"{filepath}/{mosaic_filename}")[0]
                    wcs = WCS(hdu)

                    """
                    mosaic_center = wcs.world_to_array_index(center)
                    cutout_height = min(mosaic_center[0] + height/2, hdu.shape[0]) - max(
                        mosaic_center[0] - height/2, 0)
                    cutout_width = min(mosaic_center[1] + width/2, hdu.shape[1]) - max(
                        mosaic_center[1] - width/2, 0)
                    cutout_size = cutout_height*cutout_width
                    hdus.append((hdu, cutout_size))
                    """
                    try:
                        cutout = Cutout2D(
                ...
                if hdus:
                    ...
                    hdus.sort(key=itemgetter(1), reverse=True)
                    ...
                    fits.HDUList([hdu_primary] + [hdu[0] for hdu in hdus]).writeto(
                        f"{cutout_path}/{cutout_filename}", overwrite=True,
                    )
